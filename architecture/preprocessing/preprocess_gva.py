...
from preprocessing.anonymise import anonymise_frozen  # noqa: E402
...
def read_pseudonym_key(key_path: Path, out_path: Path) -> bytes:
    """The data provider's pseudonym key. Refuses a key stored inside the repo or next to the
    artefact — it would travel with them. Length is checked by preprocessing.anonymise."""
    key_path = Path(key_path).resolve()
    forbidden = (REPO_ROOT.resolve(), Path(out_path).resolve().parent)
...
def main() -> None:
    ...
    pseudonym_key = read_pseudonym_key(args.pseudonym_key, args.out)
